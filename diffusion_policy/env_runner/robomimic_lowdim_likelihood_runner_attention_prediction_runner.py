...
class RobomimicLowdimLikelihoodRunner(BaseLowdimRunner):
    def __init__(self, 
            output_dir,
            dataset_path,
            obs_keys,
    ...
    def run(self, policy: DiffusionUnetLowdimPolicy, attention_estimator: Seq2SeqTransformer, attention_normalizer: LinearNormalizer):
    ...
    def _run_episode(self, env, policy: DiffusionUnetLowdimPolicy, attention_estimator: Seq2SeqTransformer, attention_normalizer: LinearNormalizer, device, enable_render, prefix, episode_idx, seed):
        obs = env.reset()
        policy.reset()
            ...
            for i in range(env_action.shape[0]):
                ...
                if enable_render:
                    ...
                    with torch.no_grad():
                        ...
                        attention_pred = attention_normalizer['obs'].normalize(torch.cat([torch.zeros(size=(*output.shape[:2], obs.shape[-1])).to(device), output], dim=-1))[:, :, -1].detach().cpu().numpy()
                        ...
                        attention_pred_frame.append(attention_pred[0, 0])
    ...
    def _save_visualization(self, image_frames, kl_divergence_drop_frame, attention_pred_frame, prefix, episode_idx):
        ...
        ax.plot(time_steps, kl_divergence_drop_data, '-', color='tab:blue', linewidth=1)
        ax.plot(time_steps, attention_pred_data, '-', color='tab:orange', linewidth=1)
        ...
        for t in range(len(kl_divergence_drop_data)):
            ...
            ax.scatter(t, kl_divergence_drop_data[t], color='blue', s=50)
            ax.scatter(t, attention_pred_data[t], color='orange', s=50)
            fig.canvas.draw()
            graph_img = np.frombuffer(fig.canvas.tostring_rgb(), dtype=np.uint8)
            graph_img = graph_img.reshape(fig.canvas.get_width_height()[::-1] + (3,))
