# This env runner is used to measure 2 metrics during the episode.
# 1. The time difference between last sampled moment and the moment when gripper starts closing.
# 2. The distance between the position of the object at the moment of the last sampled moment and the position of the object at the moment when gripper is fully closed.
...
class RobomimicLowdimLikelihoodDisturbanceRunner(BaseLowdimRunner):
    ...
    def run(self, policy: DiffusionUnetLowdimPolicy):
        ...
        all_triggered_times = []
        all_distance_to_normal_object_pos = []
        # Train episodes
        with h5py.File(self.dataset_path, 'r') as f:
            for i in range(self.n_train):
                ...
                max_reward, output_path, action_horizon_average_length, triggered_time, distance_to_normal_object_pos = self._run_episode(
                    env=env,
                    policy=policy,
                    device=device,
                ...
                all_distance_to_normal_object_pos.append(distance_to_normal_object_pos)
        ...
        for i in range(len(all_rewards)): # 여기 zip으로 바꾸자
            ...
            log_data[prefix+f'sim_distance_to_normal_object_pos_{seed}'] = all_distance_to_normal_object_pos[i]
            # visualize sim
    ...
    def _run_episode(self, env, policy: DiffusionUnetLowdimPolicy, device, enable_render, prefix, episode_idx, seed=42):
        ...
        return np.max(env.get_attr('reward')), output_path, np.mean(action_horizon_length_lst), triggered_time, distance_to_normal_object_pos
