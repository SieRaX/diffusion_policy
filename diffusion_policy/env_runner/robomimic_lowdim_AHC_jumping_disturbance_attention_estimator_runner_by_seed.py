...
# # Commented new version, temporary to maintain previous version
# # This runner is used to evaluate the AHC policy by seed.
# # It fixes the seed and gradually increases the c_att value to have average horizon length from 1 to 14
# import os
# import wandb
...
#         self.disturbance_generator.slice = self.slice
...
#         for eval_length in range(16, 17):
#             total_iteration = 0
#             while True:
#                 print(f"[Start]finding eval_length: {eval_length} at c_att: {c_att}")
#                 env = self.make_env()
...
#                 new_state = self.disturbance_generator.generate_state_with_disturbance(env.env.env)
#                 env.env.env.reset_to(new_state)
