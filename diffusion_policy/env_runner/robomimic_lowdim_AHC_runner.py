...
class RobomimicLowdimAHCRunner(BaseLowdimRunner):
    ...
    def run(self, policy: DiffusionUnetLowdimPolicy, c_att=0.01):
        ...
        with h5py.File(self.dataset_path, 'r') as f:
            for i in range(self.n_train):
                ...
                max_reward, output_path = self._run_episode(
                    env=env,
                    policy=policy,
                    device=device,
    ...
    def _run_episode(self, env, policy:DiffusionUnetLowdimPolicy, device, enable_render, prefix, episode_idx, seed, c_att):
        obs = env.reset()
        policy.reset()
        ...
        return np.max(env.get_attr('reward')), output_path
